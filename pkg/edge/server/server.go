...
type featureManagerMap map[string]controller.FeatureManager
...
func (s *Server) AddFeatureManager(m controller.FeatureManager) {
	s.fmm[m.GetName()] = m
}
...
func (s *Server) register(container *restful.Container) {
	...
	ws.Path(fmt.Sprintf("/%s", common.ServerRootPath)).
